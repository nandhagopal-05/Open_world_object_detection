...
class OFFLINE_AE_RPN(nn.Module):
    ...
    def asign_soft_label(self, images, gt_instances, recons_error_map_by_level):
        ...
        for batch_idx in range(len(gt_instances)):
            ...
            gt_boxes = copy.deepcopy(gt_instances[batch_idx].gt_boxes.tensor) 
            soft_label_levels = []
            ...
            # # print(bbox_level.tolist(), soft_label_levels.tolist(), gt_instances[batch_idx].gt_classes.tolist())
            
            
            ret = gt_instances[batch_idx]
            print(f"DEBUG asign_soft_label[{batch_idx}]: Before modification - type(ret) = {type(ret)}")
            print(f"DEBUG asign_soft_label[{batch_idx}]: hasattr(ret, 'image_size') = {hasattr(ret, 'image_size')}")
            if hasattr(ret, 'image_size'):
                print(f"DEBUG asign_soft_label[{batch_idx}]: ret.image_size = {ret.image_size}")
            
            ret.set("pred_boxes", Boxes(gt_instances[batch_idx].gt_boxes.tensor))
            ret.set("soft_labels", soft_label_levels)
            # Preserve the image_size attribute for postprocessing
            # image_size is a property, so we need to set _image_size directly
            ret._image_size = gt_instances[batch_idx].image_size
            
            print(f"DEBUG asign_soft_label[{batch_idx}]: After modification - type(ret) = {type(ret)}")
            print(f"DEBUG asign_soft_label[{batch_idx}]: hasattr(ret, '_image_size') = {hasattr(ret, '_image_size')}")
            if hasattr(ret, '_image_size'):
                print(f"DEBUG asign_soft_label[{batch_idx}]: ret._image_size = {ret._image_size}")
            print(f"DEBUG asign_soft_label[{batch_idx}]: hasattr(ret, 'image_size') = {hasattr(ret, 'image_size')}")
            if hasattr(ret, 'image_size'):
                print(f"DEBUG asign_soft_label[{batch_idx}]: ret.image_size = {ret.image_size}")
            
            gt_instances[batch_idx] = ret
