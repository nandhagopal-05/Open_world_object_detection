...
# ---- Load proposals (dict of image_id -> {bboxes, scores}) ----
    proposals = json.load(open(proposal_path))
    bboxes_by_id = {}
    scores_by_id = {}

    for image_id, det in tqdm(proposals.items(), ncols=80):
    # Handle both list and dict formats gracefully
        ...
        # Alternative format: list of detections
            bboxes = [d.get("bbox", d.get("bboxes", [])) for d in det]
            scores = [d.get("score", d.get("scores", 0.0)) for d in det]
        else:
            bboxes, scores = [], []

    # Convert to torch if not empty
        if len(bboxes) > 0 and len(scores) > 0:
            bboxes_by_id[image_id] = bboxes
            scores_by_id[image_id] = scores

    print(f"proposal file loaded, total images with detections: {len(bboxes_by_id)}")
    ...
    for image_id, img_data in tqdm(data_by_id.items(), ncols=80):
        ...
        # Instead of asserting, just create the mask to filter naturally!
        mask_valid_size = size <= size_img
        mask1 = size <= 0.98 * size_img
        mask2 = w / h <= 4
        mask3 = h / w <= 4
        mask4 = size >= 2000
        mask = mask_valid_size & mask1 & mask2 & mask3 & mask4
        bboxes = bboxes[mask]
        scores = scores[mask]
    ...
    if keep_type == "percent":
        ...
        print("DEBUG: len(score_record) =", len(score_record))
        if len(score_record) == 0:
            print("⚠️ No scores passed the filtering stage.")
            return
